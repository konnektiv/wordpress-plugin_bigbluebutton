...
class Bigbluebutton_Admin {
	...
	public function create_admin_menu() {
		add_menu_page(
			__( 'Rooms', 'bigbluebutton' ), __( 'Rooms', 'bigbluebutton' ), 'edit_bbb_rooms', 'bbb_room',
			'', 'dashicons-video-alt2'
		);

		if ( current_user_can( 'manage_categories' ) ) {
			add_submenu_page(
				'bbb_room', __( 'Rooms', 'bigbluebutton' ), __( 'Categories' ), 'edit_bbb_rooms',
				'edit-tags.php?taxonomy=bbb-room-category', ''
			);
		}

		add_submenu_page(
			'bbb_room', __( 'Rooms', 'bigbluebutton' ), __( 'Settings' ), 'edit_bbb_rooms',
